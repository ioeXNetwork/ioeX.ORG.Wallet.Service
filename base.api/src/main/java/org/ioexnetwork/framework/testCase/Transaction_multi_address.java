package org.ioexnetwork.framework.testCase;

import net.sf.json.JSONObject;
import org.ioexnetwork.api.SingleSignTransaction;
import org.ioexnetwork.ioex.*;
import org.ioexnetwork.framework.node.Runner;
import org.ioexnetwork.framework.restful.Restful;
...
//        启动节点，指定启动几个节点
        ...
        for (int k = 2 ;k < count ; k++ ) {
            ...
            String private_target = IOEX.getPrivateKey();
            String outputAddress_target    = IOEX.getAddressFromPrivate(private_target);
            outputList_target.add(new TxOutput(outputAddress_target,1000000));
            RawTx rawTx = IOEX.makeAndSignTx(inputList_miner.toArray(new UTXOTxInput[inputList_miner.size()]),outputList_target.toArray(new TxOutput[outputList_target.size()]));
        ...
        for (int j = 0 ; j < count ; j ++){
            String outputAddress = IOEX.getAddressFromPrivate(IOEX.getPrivateKey());
            outputList.add(new TxOutput(outputAddress,100000 ));
        }
        //多个地址的一个utxo给多个地址转账
        RawTx rawTx = IOEX.makeAndSignTx(inputList.toArray(new UTXOTxInput[inputList.size()]),outputList.toArray(new TxOutput[outputList.size()]));
